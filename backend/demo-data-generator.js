...
class FastBrainBytesDemo {
    ...
    return new Promise((resolve) => {
      const options = {
        ...
        headers: {
          ...
          'User-Agent': Math.random() > 0.25 ? 'Mobile-Android' : 'Desktop-Chrome'
        }
      };
  ...
  logScenarioEvent(scenario, requestCount) {
    ...
    const scenarioEvents = events[scenario.name] || ['📊 Generating traffic...'];
    ...
    if (scenario.errorRate > 0.1 && requestCount % 20 === 0) {
      console.log(`🚨 ALERT: High error rate (${(scenario.errorRate * 100).toFixed(1)}%)`);
    }
