...
export default function Home() {
  ...
  const updateLearningStats = (messages) => {
    ...
    for (let i = 0; i < messages.length; i++) {
      ...
      if (!msg.isUser && msg.category) {
        subjectBreakdown[msg.category] =
          (subjectBreakdown[msg.category] || 0) + 1;

        // Add to recent topics if not already included
        if (
          msg.text.length > 20 &&
          !recentTopics.some(
            (topic) => topic.text === msg.text.substring(0, 50),
          )
        ) {
          recentTopics.push({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (
        response.data.aiMessage.followUpQuestions &&
    ...
    try {
      ...
      if (response.data.users && response.data.users.length > 0) {
        ...
        const updateResponse = await axios.put(
          `http://localhost:3000/api/users/${response.data.users[0]._id}`,
          userProfile,
        );
        ...
        {[
          ...
          <button
            ...
            style={{
              ...
              border: "none",
              backgroundColor:
                activeView === tab.id ? "#2196f3" : "transparent",
          ...
          {["Math", "Science", "History"].map((subject) => (
            ...
            >
              <input
                ...
                checked={userProfile.preferredSubjects.includes(
                  subject.toLowerCase(),
                )}
                onChange={() => handlePreferenceChange(subject.toLowerCase())}
                style={{ marginRight: "5px" }}
              />
              <label htmlFor={`subject-${subject.toLowerCase()}`}>
                {subject}
              </label>
            </div>
          ))}
        ...
        >
          ...
          >
            <h3
              style={{
                color: "#333",
                margin: isSmallMobile() ? "0 0 10px 0" : 0,
              }}
            >
          ...
          >
            ...
            <div style={{ gridColumn: isSmallMobile() ? "1" : "1 / span 2" }}>
              ...
              {savedProfile.preferredSubjects &&
              savedProfile.preferredSubjects.length > 0 ? (
    ...
    >
        ...
        >
          ...
          <div style={{ textAlign: "center" }}>
            <div
              style={{ fontSize: "2rem", fontWeight: "bold", color: "#2196f3" }}
            >
              {learningStats.subjectBreakdown.math || 0}
            </div>
            <div>Math</div>
          </div>
          <div style={{ textAlign: "center" }}>
            <div
              style={{ fontSize: "2rem", fontWeight: "bold", color: "#2196f3" }}
            >
              {learningStats.subjectBreakdown.science || 0}
            </div>
            <div>Science</div>
          </div>
          <div style={{ textAlign: "center" }}>
            <div
              style={{ fontSize: "2rem", fontWeight: "bold", color: "#2196f3" }}
            >
              {learningStats.subjectBreakdown.history || 0}
            </div>
            <div>History</div>
          </div>
        </div>
      ...
      <div>
        ...
        {learningStats.recentTopics.length > 0 ? (
          ...
          <p>
            No learning activity recorded yet. Start a conversation to see your
            topics here!
          </p>
        )}
      </div>
          ...
          {["all", "math", "science", "history"].map((subject) => (
            <button
              ...
              style={{
                ...
                border: "none",
                backgroundColor:
                  subjectFilter === subject ? "#2196f3" : "transparent",
